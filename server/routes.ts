...
export async function registerRoutes(
  ...
  if (process.env["NODE_ENV"] !== "production") {
    app.post("/api/dev/save-styles", async (req, res) => {
      try {
        ...
        if (variables && Object.keys(variables).length > 0) {
          for (const [name, value] of Object.entries(variables)) {
            // Replace in both :root { ... } and .light { ... } blocks
            // Match "  --name: <old-value>;" pattern
        ...
        if (customCSS && customCSS.trim()) {
          // Append custom CSS before the final newline
          css = css.trimEnd() + "\n\n/* Dev Panel Custom CSS */\n" + customCSS.trim() + "\n";
        }
        ...
        if (commit) {
          ...
          execSync("git add client/src/index.css", { cwd: process.cwd() });
          const result = execSync(
            'git commit -m "style: update theme variables via dev panel"',
          ...
          res.json({ success: true, message: "Saved to client/src/index.css" });
        }
      } catch (error: any) {
        console.error("Dev save-styles error:", error);
        res.status(500).json({ error: error.message || "Failed to save styles" });
      }
    });

    app.post("/api/dev/save-content", async (req, res) => {
      try {
        const fs = await import("fs");
        const path = await import("path");
        ...
        // Recursively find .tsx and .ts files
        ...
        const sourceFiles = findFiles(clientSrcDir);
        ...
        const relFiles = modifiedFiles.map((f) =>
        ...
        if (commit) {
          ...
          const result = execSync(
            'git commit -m "style: update UI content via dev panel"',
            { cwd: process.cwd(), encoding: "utf-8" }
          );
          const hashMatch = result.match(/\[[\w-]+\s+([a-f0-9]+)\]/);
          const hash = hashMatch ? hashMatch[1] : "unknown";
        ...
        console.error("Dev save-content error:", error);
