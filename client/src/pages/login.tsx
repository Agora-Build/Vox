...
export function LoginForm({ variant = "user" }: LoginFormProps) {
  ...
  useEffect(() => {
    ...
    if (params.get("error") === "github_oauth_failed") {
      toast({
        title: "GitHub Sign In Failed",
        description: "Unable to sign in with GitHub. Please try again or use email.",
        variant: "destructive",
      });
    }
  }, [searchString, toast]);
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          ...
          {/* OAuth buttons - only show for regular users, not admin login */}
          {!isAdminVariant && hasOAuth && (
            <>
