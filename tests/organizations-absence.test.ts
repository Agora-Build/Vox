...
d("plugin absence causes zero persistent writes", () => {
  ...
  beforeAll(async () => {
    ...
    // PRIVATE tier deliberately, so only the eval-flow-ownership arm of the guard
  ...
  it("run-now on a team-tier PERSONAL-eval-flow schedule is 501 and writes no job", async () => {
    const before = await snapshot();
  ...
  it("run-now on an ORG-eval-flow schedule is 501 and writes no job", async () => {
    const before = await snapshot();
    const res = await request(app)
  ...
  it("console run of a PUBLIC org-owned evalFlow is 501 and writes no job", async () => {
    const before = await snapshot();
    const res = await request(app)
      .post(`/api/eval-flows/${orgEvalFlowId}/run`)
      .set("x-test-user", String(creatorId))
  ...
  it("console run of a PERSONAL evalFlow onto the TEAM tier is 501 and writes no job", async () => {
    const before = await snapshot();
    const res = await request(app)
      .post(`/api/eval-flows/${personalEvalFlowId}/run`)
      .set("x-test-user", String(creatorId))
