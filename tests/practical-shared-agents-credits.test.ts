...
describe("Task 13: practical shared-agents marketplace + credits e2e", () => {
  ...
  describe("5. Session-workflow gate interplay: consent + attestation gates precede the hold", () => {
    ...
    beforeAll(async () => {
      ...
      await createSecret(rich.session, emailSecret, "t13-test-user@example.com", { secretClass: "login" });
      await createSecret(rich.session, passwordSecret, "t13-test-password-1", { secretClass: "login" });
