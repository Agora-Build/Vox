 ...
 * populate them with `scripts/geoip-refresh.sh` (works keyless via the DB-IP
 * Lite fallback). CI and fresh checkouts have no geoip/ dir, so this file
 * skips there; it exists to prove the ladder works against actual database
 * content (field mapping, candidate derivation), which the unit tests'
 * injected fixtures cannot.
 *
...
const haveDbs =
  existsSync(path.join(GEOIP_DIR, "GeoLite2-City.mmdb")) &&
  existsSync(path.join(GEOIP_DIR, "GeoLite2-ASN.mmdb"));
...
describeGeo("detection pipeline against live GeoIP databases", () => {
  ...
  beforeAll(async () => {
    cityReader = await open<CityResponse>(path.join(GEOIP_DIR, "GeoLite2-City.mmdb"));
    asnReader = await open<AsnResponse>(path.join(GEOIP_DIR, "GeoLite2-ASN.mmdb"));
