...
describeDb("run-targets: two-level tree fields (Task 12)", () => {
  ...
  it("every agent row carries siteId/region/dispatchTier/locationTrust, with no locationSource/observedIp leak", async () => {
    const res = await authFetch(cookie, `${BASE_URL}/api/eval-flows/${evalFlowId}/run-targets?evalSetId=${evalSetId}`);
  ...
  it("mine's ?region= filter matches the EFFECTIVE region, not the (always-null) token region", async () => {
    ...
    try {
      const matching = await authFetch(cookie, `${BASE_URL}/api/eval-flows/${evalFlowId}/run-targets?evalSetId=${evalSetId}&region=${BASE_NA}`);
      ...
      const other = await authFetch(cookie, `${BASE_URL}/api/eval-flows/${evalFlowId}/run-targets?evalSetId=${evalSetId}&region=${BASE_EU}`);
