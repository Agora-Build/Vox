...
- **Broker sidecar**: a stateless service shipped as its own lean image (`vox-session-broker`, the Dockerfile's `broker` target — aeval + aeval-data + Chromium, no voice-agent-tester or daemon code) that mints a `storageState` (cookies/localStorage) by driving aeval's `setup:account` flow with the decrypted login credential. It sits on an **internal-only network** and authenticates Core's mint requests with `SESSION_BROKER_SECRET` — the login password never leaves Core → broker → target site.
