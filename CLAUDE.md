...
**Secrets follow eval flow ownership** (job-secrets endpoint): org-owned eval flow → org secrets (fenced by job creator's org membership); personal eval flow → owner's personal secrets. Built-in eval sets (`config.builtIn`) are server-controlled, admin-editable only.
...
- Suites leak resources into the dev DB and trip per-user caps (GitHub #134). Before a full run:
  ```sql
  DELETE FROM eval flows WHERE owner_id=1;
