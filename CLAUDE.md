...
- **`brokerType: 'restful'` = trusted REST execution** (design `2026-09-21-phone-vs-agent-design.md` §5): same registry, same routing, same structural withholding as auth-session. A workflow's `config.restfulTrigger` template (validated shape, `${secrets.*}`/`${phoneNumber}` placeholders) is resolved by Core from the **frozen job snapshot** at `POST /api/eval-agent/jobs/:jobId/restful` (lease-fenced; caller supplies only the `phoneNumber` variable) and dispatched to a live `restful` broker via `executeViaBroker`; everything returned to the agent is redacted with the resolved secret values then capped. Org-owned workflows resolve through `orgAllSecretsForTrustedExec` (same R3 fence, all classes — Core→broker path only).
...
- **DialF over Docker (dialfd on host, daemon in container):** `vox-upgrade.sh` auto-detects the dialfd socket and bind-mounts it + an **exchange dir at the identical path both sides** (`.env` keys: `DIALF_SOCKET`, `PHONE_EXCHANGE_DIR` default `$HOME/vox-phone-exchange`, `VOX_PHONE_NUMBER`), setting `VOX_DIALF_SOCKET`/`VOX_PHONE_EXCHANGE_DIR` in the container. The daemon stages `audio.play` files into `<exchange>/corpus/` (paths valid on both sides); point dialfd's `record_dir` INSIDE the exchange dir so recordings are readable back. Socket is file-mounted: dialfd restart ⇒ restart the container (or point dialfd's `control_socket` inside the exchange dir). `lab.trace` steps compile to DialF `log`; relative `file:` refs resolve against aeval-data — so `turn_taking_en` compiles for phone unmodified (`turn_taking_en_phone_smoke` = 3-samples/case trim for short calls; per-case analysis presets/chunking do NOT apply on the phone path).
- **Phone transport (Phase C, design `2026-09-21-phone-vs-agent-design.md` §6):** phone jobs delegate the call to **DialF ≥ v0.3.8** (`dialfd` co-located, per-user service recommended; probe = `server.info` ten_vad≠stub + `server.manifest` spec 0.1 + a connected phone — drives the `phone` capability on register/heartbeat, self-healing). `VOX_PHONE_NUMBER` env = our SIM's number. v1 supports the **outbound** mode (`config.phoneDial.number` → `job.run` dial → conversation compiled from the eval set via the corpus index → `aeval analyze` on the session dir → metrics + `callMetadata`); the trigger/inbound mode is pending DialF R7 (machine-readable serve results / `call.wait_for_ring`). One call at a time per host (DialF's sound-card lock aligns with the daemon's one-job model). Run route refuses phone workflows with neither `phoneDial` nor `restfulTrigger`.
