...
- Eval frameworks are a SEAM (`SUPPORTED_FRAMEWORKS` in `server/storage.ts` + the daemon's `executeJob` switch + `EVAL_FRAMEWORK` default; per-job override via `config.framework`); **aeval** is currently the only implementation (`aeval run scenario.yaml` → `metrics.json`). voice-agent-tester was removed 2026-09 — the `app` config key is rejected, migration 0041 parked surviving payloads as `_legacyVatApp` (whole-config secret scans skip `_legacy*` keys)
